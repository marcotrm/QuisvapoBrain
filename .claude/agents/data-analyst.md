...
1. **API del gestionale via proxy Jarvis (fonte principale)**: il server Jarvis espone in sola lettura TUTTE le API GET di SvaPro con il token già inserito:
   `curl -s "http://127.0.0.1:${PORT:-8766}/api/gestionale/<endpoint>"`
   **La mappa completa degli endpoint (vendite, ritardi/presenze, dipendenti, magazzino, clienti…) è in `40 Risorse/API Gestionale (mappa endpoint).md`: leggila PRIMA di scegliere l'endpoint.** Esempi rapidi: fatturato → `reports/store-revenue?date_from&date_to`; ritardi dipendenti → `attendance/lateness-summary`; chi è al lavoro ora → `attendance/live`; sotto scorta → `orders/stock-alerts`. Se risponde "SVAPRO_API_TOKEN non impostato", dillo all'utente: va configurato nell'ambiente del server (mai nel vault).
